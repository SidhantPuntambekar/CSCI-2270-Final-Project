...
Node* BST::deleteNode(Node *currNode, int value)
{
  ...
  else
  {
    ...
    else
    {
        ...
        Node *minimum = getMinValueNode(currNode -> right);
        currNode -> key = minimum -> key;
        currNode -> right = deleteNode(currNode -> right, minimum -> key);
    }
...
bool isValidBSTHelper(Node *crawler)
{
    ...
    if (crawler -> right != NULL && ((crawler -> right) -> key < crawler -> key))
    {
        return false;
    }
    if (crawler -> left != NULL && ((crawler -> left) -> key > crawler -> key))
    {
        return false;
    }
    return (isValidBSTHelper(crawler -> left) && isValidBSTHelper(crawler -> right));
}
